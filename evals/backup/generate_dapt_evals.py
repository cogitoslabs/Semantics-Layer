...
def main():
    ...
    # 3. Generate Retrieval Prompts & References
    retrieval_prompts = []
    ...
    for kw, ref in selected_anatomy:
        retrieval_prompts.append(f"Describe the anatomical location and primary functions of the {kw}.")
        clean_ref = re.sub(r'<!--.*?-->', '', ref).strip()
        retrieval_references.append(clean_ref)

    write_file("retrieval_prompts.json", json.dumps(retrieval_prompts, indent=2))
    write_file("retrieval_references.json", json.dumps(retrieval_references, indent=2))
    print(f"[SUCCESS] Generated retrieval_prompts.json & retrieval_references.json ({len(retrieval_prompts)} items)")
